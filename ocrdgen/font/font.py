...
# taken from https://github.com/oh-my-ocr/text_renderer/blob/master/text_renderer/bg_manager.py


class FontManager:
    def __init__(
        self, font_dir: Path, font_list_file: Optional[Path], font_size: Union[Tuple[int, int],int]
    ):
        if type(font_size)==tuple or type(font_size)==list:
            assert font_size[0] < font_size[1]
        
            self.font_size_min = font_size[0]
            self.font_size_max = font_size[1]
        else:
            self.font_size_min = font_size
            self.font_size_max = font_size
        ...
        if font_list_file is not None:
            ...
            if len(lines) == 0:
                raise PanicException(f"font list file is empty: {font_list_file}")
            ...
            for line in lines:
                ...
                if font_path.exists():
                    ...
                    raise PanicException(f"font file not exist: {font_path}")
        else:
